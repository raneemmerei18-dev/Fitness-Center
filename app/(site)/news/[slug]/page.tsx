    ...
    <main>
      <article className="mx-auto max-w-4xl px-4 md:px-6 py-20">
        <div className="mb-8 animate-fade-in-up">
          <Link href="/news" className="text-orange-600 font-bold hover:text-orange-700 flex items-center gap-1 mb-4">
            ← Back to News
          </Link>
          <div className="inline-block bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-xs font-bold mb-4">
            🔔 {new Date(item.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
          </div>
          <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-tight mb-6">{item.title}</h1>
        ...
        <div className="border-t-2 border-orange-200 py-8 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
          <div className="prose prose-lg max-w-none">
            <p className="whitespace-pre-wrap text-gray-700 leading-relaxed text-lg">{item.content}</p>
          </div>
        </div>
        ...
        <section className="mt-12 bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl p-8 animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
          <h3 className="text-2xl font-black text-slate-900 mb-4">Stay Connected</h3>
          <p className="text-gray-700 mb-6">Don&apos;t miss out on upcoming events and programs. Join our community today.</p>
