...
export default async function BlogDashboardPage() {
  ...
  const posts = (await BlogPost.find().sort({ createdAt: -1 }).lean().exec()) as unknown as Array<{
    _id: { toString(): string };
    title: string;
    slug: string;
    excerpt: string;
  ...
  return (
    <SectionCard title="Blog Management" subtitle="Create, edit, and delete blog posts">
      <form action="/api/admin/blog" method="post" className="mb-6 grid gap-2 rounded-xl border border-dashed border-slate-300 p-4">
        ...
        <button type="submit" className="justify-self-start rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Add Post</button>
      </form>

      <div className="space-y-4">
        {posts.map((post) => (
          <form key={post._id.toString()} action="/api/admin/blog" method="post" className="grid gap-2 rounded-xl border border-slate-200 p-4">
